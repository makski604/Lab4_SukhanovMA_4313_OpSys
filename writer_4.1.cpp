...
int main() {
    ...
    int processID = 0;
    if (__argc > 1) processID = std::atoi(__argv[1]);
    else {
        std::cout << "Введите номер писателя: ";
        std::cin >> processID;
    }
    ...
    std::cout << "Писатель [" << processID << "] готов" << std::endl;
    ...
    // создаём объект типа "проецируемый файл"
    ...
    std::ofstream logFile("writer_log.txt", std::ios::app); // создаём лог файл
    std::cout << "Писатель " << processID << " запущен. Нажмите Ctrl + C для выхода." << std::endl;

    while (true) {
        logFile << timeGetTime() << " : Писатель " << processID << " -> ожидание\n"; // <- логируем
        WaitForSingleObject(semEmpty, INFINITE); // ждём освобождение хотя бы одного буфера

        WaitForSingleObject(mutex, INFINITE); // Захватываем мьютекс и переходим в крит. секцию:

        /*##########################   Начало Критической секции ############################*/
        //#
        int currentPage = meta->in; //  Логируем ->:
        logFile << timeGetTime() << " : Писатель " << processID << " -> запись(стр. " << currentPage << ")\n"; 

        // Задаём длительность выполнения операции
        // случайным числом от 0.5 до 1.5 сек:
        int sleepTime = 500 + rand() % 1001;
        Sleep(sleepTime); // < --имитация записи данных в буфер

        meta->in = (currentPage + 1) % NUM_PAGES; // меняем крит. данные (переход на след. стр.)
        logFile << timeGetTime() << " : Писатель " << processID << " -> освобождение\n"; // <- логируем
        //#
        /*##########################   Конец Критической секции ############################*/

        ReleaseMutex(mutex); // Выходим из крит. секции и освобождаем мьютекс

        ReleaseSemaphore(semFull, 1, NULL); // +1 заполненная страница
        Sleep(1000);
    }
